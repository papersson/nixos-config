...
- **Phase 2 (*arr stack via nixarr)** — in progress; download VPN + indexers verified. `nixarr` flake input + module wired into `hosts/z840`. Mullvad WireGuard config encrypted at `secrets/z840-wireguard.conf` (binary sops secret, decrypted at activation by the host key). qBittorrent confined to the `wg` namespace; **confinement verified** — host exits on the ISP (Sweden) while the qBittorrent namespace exits Mullvad (Germany/Frankfurt, `mullvad_exit_ip:true`). Sonarr / Radarr / Prowlarr / Bazarr run on the host. **Prowlarr stays on the host, NOT in the tunnel** — see the indexer-DNS gotcha below; the host resolves via Cloudflare and `services.flaresolverr` solves Cloudflare challenges. Indexers (1337x via FlareSolverr, The Pirate Bay direct) test green. **Remaining**: repoint Jellyfin libraries; connect Sonarr/Radarr to qBittorrent + set root folders (`/tank/media/library/{movies,shows}`) + dual-quality profiles; Bazarr subtitle provider; validate an end-to-end grab. Usenet (Phase 2.5) deferred until that passes.
...
3. ✅ Sonarr, Radarr, Prowlarr, Bazarr enabled on the host; qBittorrent with `vpn.enable = true` (only the downloader is confined). Confinement verified: host exits Sweden/ISP, qBittorrent namespace exits Germany/Mullvad. Recyclarr deferred.
4. ⬜ Sonarr/Radarr root folders → `/tank/media/library/{shows,movies}`. Download client → qBittorrent at `192.168.15.1:8085` (the VPN namespace address, reachable from the host). (qBittorrent default download dir is `/tank/media/qbittorrent`, same dataset as the library, so imports hardlink.)
...
7. ✅ Prowlarr indexers: added on the host (1337x via FlareSolverr, The Pirate Bay direct), synced to Sonarr/Radarr via the Apps panel (`http://localhost:8989` / `7878`, API keys). FlareSolverr added as a tagged indexer proxy (`http://localhost:8191`); Cloudflare-fronted indexers carry the matching tag.
...
> **Gotcha — indexer DNS, the long way round.** Public torrent indexers are unreachable from this host by default, for three stacked reasons, fixed in order:
> 1. **Telia DNS-blocks them.** Swedish ISPs block piracy sites at their resolver. The host must not use the Telia router (`192.168.1.1`) for DNS — `networking.nameservers` is set to Cloudflare (`1.1.1.1`/`1.0.0.1`), since glibc stops at the first resolver that answers and a blocking one in front shadows the rest.
> 2. **A ghost subscriber re-prepended Telia.** Even with the nameserver list changed, `resolvconf` let a transient boot-time source put `192.168.1.1` back at the top of `/etc/resolv.conf`. Fix: `networking.resolvconf.enable = false` so a static Cloudflare `resolv.conf` is written from the list.
> 3. **Confining Prowlarr to the VPN did NOT help** (an abandoned approach): Mullvad force-redirects all port-53 DNS to its own resolver, which *also* filters these sites, and only DoH (443) escapes it. So Prowlarr stays on the host; only qBittorrent is tunnelled. Trade-off: indexer lookups ride the ISP line (metadata), downloads stay on Mullvad.
> 4. **Cloudflare bot-challenge.** Once DNS resolved, indexers like 1337x returned a Cloudflare "are you a bot" page. `services.flaresolverr` (headless browser on `localhost:8191`) solves it; Prowlarr uses it as a tagged indexer proxy.
