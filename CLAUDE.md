...
- `pkgs/` — local package derivations applied via the overlay in `flake.nix` (currently `codex.nix`: the Codex CLI from OpenAI's official release binary, overriding `pkgs.codex`). Bump with the `codex-bump` shell function.
...
- **Fast-moving packages** come from the `pkgs.unstable.*` overlay (defined in `flake.nix`). Bump with `nix flake update nixpkgs-unstable`. When even unstable lags too far (Claude Code, Codex), package the vendor's prebuilt binary instead: `nix-claude-code` input for Claude, `pkgs/codex.nix` for Codex.
...
- **Adding a papershop skill**: skills from the `papershop` flake input are *not* auto-discovered — each is wired in explicitly in `home/patrikpersson/claude.nix`. To install a new one: (1) add a `home.file.".claude/skills/<name>".source = "${inputs.papershop}/<name>/skills/<name>";` line (symlink the whole skill dir so its `modules/`/`reference.md` resolve next to `SKILL.md`; worklog also needs its `home.packages` CLI entry); (2) `nix flake update papershop` to bump the lock past the revision that added the skill; (3) rebuild. A skill that's "missing" is almost always one of these two steps left undone — the lock is stale, or the install line was never added.
