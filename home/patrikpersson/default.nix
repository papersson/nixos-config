...
{
  imports = [ ./palette.nix ./nvim.nix ./hyprland.nix ./theming.nix ./desktop-shell.nix ./claude.nix ./codex.nix ];
  ...
  programs.zsh = {
    ...
    initContent = ''
      ...
      claude-bump() {
        ( cd /etc/nixos && nix flake update nix-claude-code ) || return 1
        nh os switch || return 1
        git -C /etc/nixos commit -m "claude: bump nix-claude-code to latest" flake.lock
      }
